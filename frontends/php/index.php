...
if (isset($_REQUEST['enter']) && $_REQUEST['enter'] == _('Sign in')) {
	...
	if ($loginSuccess) {
		...
		if ($request) {
			$test_request = parse_url($request);

			$request = (array_key_exists('path', $test_request) && file_exists('./'.basename($test_request['path'])))
				? $test_request['path'].(array_key_exists('query', $test_request) ? '?'.$test_request['query'] : '')
				: '';
		}

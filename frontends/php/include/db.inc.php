...
/**
 * Check database, table and field charset and collation.
 * Support MySQL, PostgreSQL, OracleDB.
 *
 * @throws Exception
 *
 * @return boolean
...
function checkDatabaseCharset() {
	...
	if ($DB['TYPE'] == ZBX_DB_MYSQL || $DB['TYPE'] == ZBX_DB_POSTGRESQL) {
		$db_charset = getDefaultDatabaseCharset($DB['TYPE'], $DB['DATABASE']);
		if ($db_charset != ZBX_DB_DEFAULT_CHARSET) {
			throw new Exception(_s('Incorrect default charset for Zabbix database: %1$s.',
				_s('"%1$s" instead "%2$s"', $db_charset, ZBX_DB_DEFAULT_CHARSET)
			));
		}
	}
	...
	switch ($DB['TYPE']) {
		case ZBX_DB_MYSQL:
			$query = DBselect('SELECT table_name, column_name, character_set_name, collation_name'.
				' FROM information_schema.columns'.
				' WHERE '.dbConditionString('table_schema', [$DB['DATABASE']]).
				' AND '.dbConditionString('table_name', array_keys($tables)).
				' AND '.dbConditionString('data_type', ['text', 'varchar', 'longtext'])
			);
			while ($row = DBfetch($query)) {
				if (strtoupper($row['character_set_name']) != ZBX_DB_DEFAULT_CHARSET) {
					throw new Exception(_s('Incorrect default charset for Zabbix table "%1$s" field "%2$s": %3$s.',
						$row['table_name'], $row['column_name'],
						_s('"%1$s" instead "%2$s"', $row['character_set_name'], ZBX_DB_DEFAULT_CHARSET)
					));
				}
				if ($row['collation_name'] != ZBX_DB_MYSQL_DEFAULT_COLLATION) {
					throw new Exception(_s('Incorrect default charset collation for Zabbix table "%1$s" field "%2$s": %3$s.',
						$row['table_name'], $row['column_name'],
						_s('"%1$s" instead "%2$s"', $row['collation_name'], ZBX_DB_MYSQL_DEFAULT_COLLATION)
					));
				}
			}
			break;
		...
		case ZBX_DB_POSTGRESQL:
			// Getting oid for table.
			$row = DBfetch(DBselect('SELECT oid FROM pg_namespace'.
				' WHERE '.dbConditionString('nspname', [$DB['SCHEMA'] ?: 'public']))
			);
			$oid = $row['oid'];
			 ...
			 */
			$query = DBselect('SELECT c.relname, a.attname, l.collname FROM pg_attribute AS a'.
				' LEFT JOIN pg_class AS c ON c.relfilenode = a.attrelid'.
				' LEFT JOIN pg_collation AS l ON l.oid = a.attcollation'.
				' WHERE '.dbConditionInt('atttypid', [25, 1043]).' AND '.dbConditionInt('c.relnamespace', [$oid]).
				' AND c.relam = 0 AND '.dbConditionString('c.relname', array_keys($tables))
			);
			while ($row = DBfetch($query)) {
				if ($row['collname'] != 'default') {
					throw new Exception(_s('Incorrect default charset collation for Zabbix table "%1$s" field "%2$s": %3$s.',
						$row['relname'], $row['attname'], _s('must be "%1$s"', ZBX_DB_DEFAULT_CHARSET)
					));
				}
			}

			// PostgreSQL automatic convert data to coincide client encoding.
			$client_encoding = DBfetch(DBselect('show client_encoding;'));
			if ($client_encoding['client_encoding'] != ZBX_DB_DEFAULT_CHARSET) {
				throw new Exception(_s('Incorrect client encoding for PostgreSQL: %1$s.',
					_s('"%1$s" instead "%2$s"', $client_encoding['client_encoding'], ZBX_DB_DEFAULT_CHARSET)
				));
			}

			// PostgreSQL automatic convert data to coincide server encoding.
			$server_encoding = DBfetch(DBselect('show server_encoding;'));
			if ($server_encoding['server_encoding'] != ZBX_DB_DEFAULT_CHARSET) {
				throw new Exception(_s('Incorrect server encoding for PostgreSQL: %1$s.',
					_s('"%1$s" instead "%2$s"', $server_encoding['server_encoding'], ZBX_DB_DEFAULT_CHARSET)
				));
			}
			break;

		case ZBX_DB_ORACLE:
			$query = DBselect('SELECT * FROM NLS_DATABASE_PARAMETERS'.
				' WHERE '.dbConditionString('parameter', ['NLS_CHARACTERSET', 'NLS_NCHAR_CHARACTERSET'])
			);
			while ($row = DBfetch($query)) {
				if ($row['value'] != ZBX_DB_DEFAULT_CHARSET) {
					throw new Exception(_s('Incorrect parameter "%1$s" value: %2$s.',
...
/**
 * Get default charset for database.
 * Support only MySQL and PostgreSQL.
 *
 * @param string $type     $DB['TYPE']
 * @param string $db_name  $DB['DATABASE']
 *
 * @return mixed
 */
function getDefaultDatabaseCharset($type, $db_name) {
	switch($type) {
		case ZBX_DB_MYSQL:
			$sql = 'SELECT default_character_set_name db_charset FROM information_schema.schemata'.
				' WHERE '.dbConditionString('schema_name', [$db_name]);
			break;

		case ZBX_DB_POSTGRESQL:
			$sql = 'SELECT pg_encoding_to_char(encoding) db_charset FROM pg_database'.
				' WHERE '.dbConditionString('datname', [$db_name]);
			break;
	...
	return strtoupper($result['db_charset']);
}

...
class CHttpTestManager {
		...
		foreach ($httpTests as $httpTest) {
			...
			foreach ($hostHttpTests as $hostId => $hostHttpTest) {
				...
				if (isset($hostHttpTest['byTemplateId'][$httpTestId])) {
					...
					// need to check templateid here too in case we update linked http test to name
					// that already exists on linked host
	 ...
	 * @param array $exHttpTest properties to compare with
	 *
	 * @return bool
	 */

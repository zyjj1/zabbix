...
function check_field(&$fields, &$field, $checks) {
	...
	if ((is_null($exception) || $except) && $validation && !calc_exp($fields, $field, $validation)) {
		...
		elseif ($validation === VALID_CSRF_TOKEN) {
			info(_('Request contains invalid or already utilized CSRF token. Please refresh the page and try again.'));
		}
		else {
...
function check_fields(&$fields, $show_messages = true) {
	 ...
	 *
	 * CSRF token is added only to requests which are made using POST HTTP method.
	 */
	if (!array_key_exists('HTTP_X_REQUESTED_WITH', $_SERVER) && $_SERVER['REQUEST_METHOD'] === 'POST') {
		$system_fields['csrf_token'] =	[T_ZBX_STR, O_MAND, P_SYS, VALID_CSRF_TOKEN, null];
	}
...
/**
 * Validate either the value is valid CSRF token generated by method createCSRFToken(). If token is valid, method will
 * delete it to avoid it to be used repeatedly.
 *
 * @param str
 *
 * @return bool
...
function is_valid_csrf_token($value) {
	if (!CSession::keyExists('csrf_tokens')) {
		return false;
	}

	$csrf_tokens = CSession::getValue('csrf_tokens');
